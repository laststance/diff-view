...
// ContentLoading and ErrorMessage components removed
...
export const DiffViewer: React.FC<DiffViewerProps> = ({ className }) => {
  ...
  const computeDiff = useCallback(
    async (left: string, right: string) => {
      ...
      const { setError, clearError, addErrorToHistory, setLoadingState } = useAppStore.getState();
  ...
  try {
    return (
      ...
      >
        ...
        {/* Empty state */}
        {!leftContent && !rightContent && !loadingStates.diffComputation && (
          <div
    ...
    // Create error object for critical errors
    const criticalError: AppError = {
      type: 'unknown',
      message: 'Critical error in diff viewer',
      details:
